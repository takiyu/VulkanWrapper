...
void RunExampleApp10(const vkw::WindowPtr& window,
    ...
    // Create context
    vkw::ContextInfo vk_ctx_ci;
    vk_ctx_ci.app_name = "VKW Example 10";
    vkw::ContextPtr vk_ctx = vkw::Context::Create();
    vk_ctx->init(vk_ctx_ci, window);

    // Create images
    const auto RGBA8_FMT = vk::Format::eA8B8G8R8SnormPack32;
    const auto D16_FMT = vk::Format::eD16Unorm;
    const auto RGBA32_FMT = vk::Format::eR32G32B32A32Sfloat;
    const auto R32_FMT = vk::Format::eR32Sfloat;
    const auto SWP_SIZE = vk::Extent2D(0, 0);  // Use swapchain's size
    // Create G buffer 0 (color)
    auto gbuf_col_img_pack = vk_ctx->createColorImagePack(
            RGBA8_FMT, SWP_SIZE, vk::ImageUsageFlagBits::eInputAttachment |vk::ImageUsageFlagBits::eColorAttachment);
    // Create G buffer 1 (normal)
    auto gbuf_nor_img_pack = vk_ctx->createColorImagePack(
            RGBA8_FMT, SWP_SIZE, vk::ImageUsageFlagBits::eInputAttachment |vk::ImageUsageFlagBits::eColorAttachment);
    // Create depth buffer
    auto depth_img_pack = vk_ctx->createDepthImagePack(
            D16_FMT, SWP_SIZE);

    // Create uniform buffer
    auto uniform_buf_pack = vk_ctx->createHostVisibBuffer(
            sizeof(glm::mat4),
            vk::BufferUsageFlagBits::eUniformBuffer);

    // Create color texture
    auto color_img_pack = vk_ctx->createColorImagePack(
            RGBA32_FMT, {mesh.color_tex_w, mesh.color_tex_h},
            vk::ImageUsageFlagBits::eSampled |
                    vk::ImageUsageFlagBits::eTransferDst);
    auto color_tex_pack = vk_ctx->createTexturePack(color_img_pack);
    // Create bump texture
    auto bump_img_pack = vk_ctx->createColorImagePack(
            R32_FMT, {mesh.bump_tex_w, mesh.bump_tex_h},
            vk::ImageUsageFlagBits::eSampled |
                    vk::ImageUsageFlagBits::eTransferDst);
    auto bump_tex_pack = vk_ctx->createTexturePack(bump_img_pack);

    // Create pipelines
    vkw::DefererStageInfo stage_ci0 = {
        uint32_t(~0), {},  // No input attachments
        1, {color_tex_pack, bump_tex_pack},
        0, {uniform_buf_pack},
        {gbuf_col_img_pack, gbuf_nor_img_pack}, depth_img_pack,
            {{0, sizeof(Vertex), vk::VertexInputRate::eVertex}},
            {{0, 0, vk::Format::eR32G32B32Sfloat, 0},
             {1, 0, vk::Format::eR32G32B32Sfloat, sizeof(float) * 3},
             {2, 0, vk::Format::eR32G32Sfloat, sizeof(float) * 6}},
            {},
            VERT_SOURCE1, FRAG_SOURCE1
    };
    vkw::DefererStageInfo stage_ci1 = {
        0, {gbuf_col_img_pack, gbuf_nor_img_pack},
        uint32_t(~0), {},  // No texture inputs
        uint32_t(~0), {},  // No uniform buffers
        {nullptr /* swapchain */ }, nullptr /* no depth */
    };

    vkw::DefererPtr deferer = vkw::Deferer::Create();
    deferer->init(vk_ctx, vk_ctx->m_swapchain_pack->size, {stage_ci0, stage_ci1});
